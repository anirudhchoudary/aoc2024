...
def check_blocking_positions(file_path):
    G = 70
    byteCtr = 1024
    ...
    for i, (x, y) in enumerate(coordinates[byteCtr:]):
        ...
        if steps == -1:
            print(f"Path is blocked by position: ({x}, {y})")
        print("============")
        for array in grid:
            print(''.join(array))
        return
    ...
    print("No blocking positions found. Path remains clear.")

def main():
    file_path = 'd18-input.txt'
    check_blocking_positions(file_path)
